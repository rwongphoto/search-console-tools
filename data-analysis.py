...
# Imports moved inside the function where they are needed
...
def generate_topic_label(queries_in_topic):
    ...
    if bigram_list:
        ...
        return label.title() if label else "Topic Cluster"
...
def calculate_pct_change(val_after, val_before):
    val_after = pd.to_numeric(val_after, errors='coerce'); val_before = pd.to_numeric(val_before, errors='coerce')
...
def google_search_console_analysis_page():
    ...
    if uploaded_file_before is not None and uploaded_file_after is not None:
        ...
        try:
            ...
            status_text.text("Reading CSV files..."); df_before = pd.read_csv(uploaded_file_before); df_after = pd.read_csv(uploaded_file_after)
            ...
            df_before_renamed = df_before.rename(columns={"Top queries": "Query", "Position": "Average Position"}); df_after_renamed = df_after.rename(columns={"Top queries": "Query", "Position": "Average Position"})
            cols = st.columns(4)
            ...
            status_text.text("Merging datasets & calculating YoY..."); merged_df = pd.merge(df_before, df_after, on="Top queries", suffixes=("_before", "_after"), how="inner")
            ...
            intent_distribution = merged_df['Intent'].value_counts(normalize=True, dropna=False) * 100
            ...
            try:
                ...
                if query_matrix_lda.shape[1] > 0:
                    ...
                    if actual_n_topics >= 2:
                        ...
                        lda_queries_model.fit(query_matrix_lda); query_topic_labels = lda_queries_model.transform(query_matrix_lda).argmax(axis=1)
                        ...
                        for topic_idx, topic_comp in enumerate(lda_queries_model.components_):
                            top_feature_indices = topic_comp.argsort()[-10:][::-1]; topic_bigrams = [feature_names_queries_lda[i] for i in top_feature_indices]
                ...
                else: st.warning("LDA Warning: No bi-gram features found.")
            ...
            if not intent_agg_display.empty: # Plot Intent YoY %
                st.markdown("##### Intent YoY % Change (Clicks & Impressions)")
                try:
                    intent_pct_melt = intent_agg_display.melt(id_vars='Intent', value_vars=['Clicks YoY %', 'Impressions YoY %'], var_name='Metric', value_name='YoY % Change'); intent_pct_melt['Metric'] = intent_pct_melt['Metric'].str.replace(' YoY %', '')
                    plot_data_intent_pct = intent_pct_melt.dropna(subset=['YoY % Change'])
                    if not plot_data_intent_pct.empty:
                        intent_order = ["Informational", "Navigational", "Commercial", "Transactional", "Unknown"]; fig_intent_pct = px.bar(plot_data_intent_pct, x='Intent', y='YoY % Change', color='Metric', barmode='group', title='YoY % Change by Search Intent', labels={'YoY % Change': 'YoY Change (%)'}, category_orders={"Intent": [i for i in intent_order if i in plot_data_intent_pct['Intent'].unique()]}, text_auto='.1f')
                        fig_intent_pct.update_traces(textposition='outside'); st.plotly_chart(fig_intent_pct, use_container_width=True)
                    else: st.warning("No valid YoY % data available to plot for search intents.")
                except Exception as e: st.error(f"Error generating Intent YoY % plot: {e}"); st.error(traceback.format_exc())
            else: st.warning("No aggregated intent data for visualization.")
